...
#include "assign_weights_from_gff.h"
...
int assign_weights_from_gff(char *name_fileinputgff,
	...
	SGZip file_gff_gz,file_gff2_gz;
	...
	if(file_gff) {
        ...
        while(1)
        {
			...
			/*	i++;*/
			/*	if(i >= SIZE_ROW) while((j = fzgetc(file_gff,&file_gff_gz)) != 0  && j != 10 && j != 13 && (fzeof(file_gff, &file_gff_gz)) != 1); */
			/*}*/
			/*row[i] = 0;*/
			...
			if(j < 9) continue;
            /*********** filter by scaffold *********/
            if(strcmp(fieldsgff[nrows].filename,chr_name)!=0/* && chr_name_pass == 0*/)
                continue;
            /*if(strcmp(fieldsgff[nrows].filename,chr_name)!=0 && chr_name_pass == 1)
                break;*/
            chr_name_pass = 1;
            /****************************************/
            /*filtering ...*/
			...
			if(fzeof(file_gff, &file_gff_gz)) break;
		}
		free(row);
		free(f);
		fzclose(file_gff,&file_gff_gz);
		...
		for(n=0;n<9;n++) {
			switch(n) {
				case 0:
					fieldsgff[nrows].filename[0] = '\0';
					strncat(fieldsgff[nrows].filename,chr_name/*name_fileinputgff*/,256*sizeof(char));
					break;
				case 1:
					fieldsgff[nrows].source[0]='\0';
					break;
		...
		*/
		...
		/*
		 once all rows are counted and kept, look at each gene_id transcripts. Take the "criteria_transcripts" (max, min, first. long).
		...
		for(n=0;n<nrows;) {
			...
			if(overlap_nf==1) {
				...
				while(l <= end-start) {
                    ...
                    jj = (int)l;
					while(matrix_coding[ntransc][jj] != 0 && jj <= end-start) {jj++;}
					if(jj-1<=end-start && matrix_coding[ntransc][jj-1] != 0) {
		...
		*name_fileinputgff2 = '\0';
		...
		for(n=0;n<nrows;n++) {
			fzprintf(file_gff2,&file_gff2_gz,"%s\t",fieldsgff2[n].filename);
			fzprintf(file_gff2,&file_gff2_gz,"%s\t",fieldsgff2[n].source);
			fzprintf(file_gff2,&file_gff2_gz,"%s\t",fieldsgff2[n].feature);
			fzprintf(file_gff2,&file_gff2_gz,"%ld\t",fieldsgff2[n].start+1);
			fzprintf(file_gff2,&file_gff2_gz,"%ld\t",fieldsgff2[n].end+1);
			fzprintf(file_gff2,&file_gff2_gz,"%s\t",fieldsgff2[n].score);
			fzprintf(file_gff2,&file_gff2_gz,"%c\t",fieldsgff2[n].strand[0]);
            if(fieldsgff2[n].frame[0]=='1') fzprintf(file_gff2,&file_gff2_gz,"2\t");
            else {
                if(fieldsgff2[n].frame[0]=='2') fzprintf(file_gff2,&file_gff2_gz,"1\t");
                else {
                    fzprintf(file_gff2,&file_gff2_gz,"%c\t",fieldsgff2[n].frame[0]);
                }
            }
			fzprintf(file_gff2,&file_gff2_gz,"gene_id %s;",fieldsgff2[n].gene_id);
			fzprintf(file_gff2,&file_gff2_gz,"transcript_id %s;",fieldsgff2[n].transcript_id);
			fzprintf(file_gff2,&file_gff2_gz,"\n");
		}
		fzclose(file_gff2,&file_gff2_gz);
        ...
        */
        ...
        /*Read tfa file: defined static int n_samp (total number of samples) and nsamoutg=n_samp-outgroup_presence+1*/
        /*first obtain the number of samples and the outgroup, if population is defined*/
        if(first==0) {
        ...
        n_samp = nseq;
        nsamoutg=n_samp-outgroup_presence;
        ...
        /*second: for each gene where the codon positions have to be defined for weights, go directly to the positions using the indexed file*/
        /*third: read tfa file for that specific gene and keep into a matrix (char *DNA_matr)*/
        /*fourth: change functions to use the relative value of the positions analyzed*/
        /*allocated matrix_segrpos and matrix_sizepos: define the values and send to main file. There they will be printed*/
			...
			if(strcmp(subset_positions,"synonymous") == 0 || strcmp(subset_positions,"nonsynonymous") == 0 || strcmp(subset_positions,"silent") == 0 || strcmp(subset_positions,"silent") == 0 || strcmp(subset_positions,"0-fold") == 0 || strcmp(subset_positions,"2-fold") == 0 || strcmp(subset_positions,"3-fold") == 0 || strcmp(subset_positions,"4-fold") == 0) {
				...
				if(cstrand[0] == '*') {
                    ...
                    if(startframe != -1 || endframe != -1) {
                        /*read TFA file: Only read the given positions*/
                        ...
                        /*
                        if(n_sitei != endDNAmatr-startDNAmatr+1)
                            printf("WARNING!\n");
                        */
                    }
					/*check frame*/
					...
					for(ii=start;ii*k <= end*k;ii += k) {
                        ...
                        if(tripletnsamp(cod3n,DNA_matr,cstrand[0],cmat+startDNAmatr,n_samp,
                                        (long int)/*n_site*/endDNAmatr-startDNAmatr+1,
                                        end<endDNAmatr-startDNAmatr?end:endDNAmatr-startDNAmatr,
        ...
        /*eliminate all positions with less than two individuals in the population*/
        /*OR less than 1 individual + 1 outg*/
        
        int countNoNs,countNoNoutg;
        for(ii2=0;ii2<(long int)n_site;ii2+=100000) {
            if(ii2+100000 >= n_site) ii2end = n_site;
            else ii2end = ii2 + 100000;
            /**Here CALCULATE the DNA_matr in BLOCKS of 100Kb, for example...*/
            DNA_matr = (char *)calloc(ii2end-ii2,sizeof(char));
            /*read TFA file: Only read the given positions*/
            if(function_read_tfasta(file_input,file_input_gz,file_input_gz_index,file_logerr,file_logerr_gz,
                                    ii2+1,ii2end+1,n_samp,&n_sitei,&DNA_matr,chr_name,first) == 0) {
                fzprintf(file_logerr,file_logerr_gz,"Error reading tfa file: %s:%ld-%ld",chr_name,startframe,endframe);
                return(0);
            }
            for(ii=ii2;ii<ii2end;ii++) {
                if(matrix_sizepos[ii] > 0) {
                    if(outgroup_presence == 0) {
                        countNoNs = 0;
                        for(j=0;j<n_samp;j++) {
                            if(DNA_matr[n_sitei*j+ii-ii2] < '5')
                                countNoNs += 1;
                        }
                        if(countNoNs < 2)
                            matrix_sizepos[ii] = 0;
...
int function_read_tfasta(FILE *file_input,SGZip *file_input_gz,struct SGZIndex *index_input,FILE *file_logerr,SGZip *file_logerr_gz,long int init_site,long int end_site,int n_sam, long int *n_site, char **DNA_matr,char *chr_name,int first)
{
    static char c[1];
    static char line[MSP_MAX_COL_LINE];
    ...
    *n_site = 0;
    ...
    free(*DNA_matr);
    if((*DNA_matr = (char *)calloc(n_site[0]*(long long)n_sam,sizeof(char))) == 0) {
